...
class AIAssistant:
    ...
    def chain_of_thought_reasoning(self, complex_task: str):
    ...
    def sprint_planner(self):
        ...
        # Ask for sprint details
        sprint_duration = input("Sprint duration (days): ") or "14"
        team_size = input("Team size: ") or "5"
        priority_areas = input("Priority areas (comma-separated): ") or "bug fixes, performance, new features"
        
        messages = [
            {"role": "system", "content": "You are an agile project manager. Create realistic sprint plans."},
            {"role": "user", "content": f"""
            Create a {sprint_duration}-day sprint plan for a team of {team_size} developers.
            
            Project context: {context[:5000]}...
        ...
        if response:
            ...
            save_plan = input("\n💾 Save sprint plan? (y/n): ")
            if save_plan.lower() == 'y':
                plan_filename = f"sprint_plan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
                self.write_to_file(plan_filename, f"# Sprint Plan\n\n{plan}")
                print(f"✅ Plan saved to {plan_filename}")

    def technical_debt_calculator(self):
        """Calculate and track technical debt"""
        print("💰 Technical Debt Calculator")
        print("=" * 30)
        
        debt_items = []
        
        # Analyze code quality issues
        quality_metrics = self.assess_code_quality()
        if quality_metrics.get("todos", 0) > 0:
            debt_items.append({
                "type": "TODO/FIXME Comments",
                "count": quality_metrics["todos"],
                "effort": quality_metrics["todos"] * 0.5,  # 0.5 hours per TODO
                "cost": quality_metrics["todos"] * 50  # $50 per TODO
            })
        
        if quality_metrics.get("large_files", 0) > 0:
            debt_items.append({
                "type": "Large Files (>500 lines)",
                "count": quality_metrics["large_files"],
                "effort": quality_metrics["large_files"] * 8,  # 8 hours per large file
                "cost": quality_metrics["large_files"] * 800  # $800 per large file
            })
        
        # Analyze test coverage debt
        test_metrics = self.assess_test_coverage()
        coverage_ratio = float(test_metrics.get("coverage_ratio", "0").rstrip('%')) / 100
        if coverage_ratio < 0.8:  # Less than 80% coverage
            missing_coverage = (0.8 - coverage_ratio) * test_metrics.get("component_files", 0)
            debt_items.append({
                "type": "Missing Test Coverage",
                "count": int(missing_coverage),
                "effort": int(missing_coverage * 2),  # 2 hours per missing test
                "cost": int(missing_coverage * 200)  # $200 per missing test
            })
        ...
        print("=" * 40)
        print(f"Total Effort Required: {total_effort:.1f} hours")
        print(f"Estimated Cost: ${total_cost:,}")
        
        if debt_items:
            print(f"\n🔍 Debt Items:")
            for item in debt_items:
                print(f"  • {item['type']}: {item['count']} items")
                print(f"    Effort: {item['effort']:.1f} hours | Cost: ${item['cost']:,}")
        
        # Generate debt reduction plan
        if debt_items:
            reduction_plan = input("\n🤖 Generate debt reduction plan? (y/n): ")
            if reduction_plan.lower() == 'y':
                self.generate_debt_reduction_plan(debt_items, total_effort, total_cost)

    def generate_debt_reduction_plan(self, debt_items, total_effort, total_cost):
        """Generate technical debt reduction plan"""
        messages = [
            {"role": "system", "content": "You are a technical debt expert. Create actionable reduction plans."},
            {"role": "user", "content": f"""
            Create a technical debt reduction plan for:
            
            Debt Items:
            {chr(10).join([f"- {item['type']}: {item['count']} items ({item['effort']} hours, ${item['cost']})" for item in debt_items])}
            ...
            2. Phased approach (short-term, medium-term, long-term)
        ...
        if response:
            ...
            save_plan = input("\n💾 Save reduction plan? (y/n): ")
            if save_plan.lower() == 'y':
                plan_filename = f"technical_debt_plan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
                self.write_to_file(plan_filename, f"# Technical Debt Reduction Plan\n\n{plan}")
                print(f"✅ Plan saved to {plan_filename}")
