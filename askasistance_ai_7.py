...
class Plugin:
    ...
    def execute(self, assistant, *args, **kwargs):
...
class PluginManager:
    """Manages plugins for the AI Assistant"""
    def __init__(self):
    ...
    def ensure_plugin_directory(self):
        ...
        if not os.path.exists(self.plugin_directory):
            ...
            self.create_sample_plugin()
    
    def create_sample_plugin(self):
        """Create a sample plugin file for reference"""
        sample_plugin_content = '''
"""
Sample Plugin for AI Assistant
"""
from ai_assistant import Plugin
...
class SamplePlugin(Plugin):
    ...
    def execute(self, assistant, *args, **kwargs):
        ...
        print("You can create your own plugins by following this template.")
        
        # Access assistant methods if needed
        # assistant.analyze_code()
        
        return "Sample plugin completed successfully"

# Register the plugin
def register_plugin():
    return SamplePlugin()
'''
        sample_plugin_path = os.path.join(self.plugin_directory, "sample_plugin.py")
        if not os.path.exists(sample_plugin_path):
            with open(sample_plugin_path, 'w', encoding='utf-8') as f:
                f.write(sample_plugin_content.strip())
            print(f"✅ Created sample plugin at {sample_plugin_path}")
                ...
                try:
                    ...
                    if hasattr(module, 'register_plugin'):
                        ...
                        if isinstance(plugin_instance, Plugin):
                            ...
                            print(f"⚠️  Invalid plugin in {filename}: register_plugin must return Plugin instance")
                    else:
                        print(f"⚠️  No register_plugin function found in {filename}")
    ...
    def execute_plugin(self, name: str, assistant, *args, **kwargs) -> Any:
        ...
        try:
            ...
            result = plugin.execute(assistant, *args, **kwargs)
...
class AIAssistant:
    ...
    def plugin_system_menu(self):
        """Plugin system management menu"""
        while True:
            print("\n🔌 Plugin System Management")
            print("=" * 40)
            print("1. List Plugins")
            print("2. Enable Plugin")
            print("3. Disable Plugin")
            print("4. Execute Plugin")
            print("5. Refresh Plugins")
            print("6. Create New Plugin Template")
            ...
            if choice == '1':
                ...
                plugin_name = input("Enter plugin name to enable: ")
                self.plugin_manager.enable_plugin(plugin_name)
            elif choice == '3':
                plugin_name = input("Enter plugin name to disable: ")
                self.plugin_manager.disable_plugin(plugin_name)
            elif choice == '4':
                plugin_name = input("Enter plugin name to execute: ")
                self.plugin_manager.execute_plugin(plugin_name, self)
            elif choice == '5':
                self.plugin_manager.discover_plugins()
            elif choice == '6':
    ...
    def create_plugin_template(self):
        ...
        plugin_name = input("Enter plugin name (e.g., 'Code Formatter'): ").strip()
        if not plugin_name:
            print("❌ Plugin name cannot be empty")
            return
        
        # Convert to valid Python class name
        class_name = ''.join(word.capitalize() for word in plugin_name.replace('-', ' ').replace('_', ' ').split())
        file_name = plugin_name.lower().replace(' ', '_').replace('-', '_')
        
        template_content = f'''
"""
{plugin_name} Plugin for AI Assistant
"""
from ai_assistant import Plugin
...
class {class_name}(Plugin):
    def __init__(self):
        super().__init__(
            name="{plugin_name}",
            description="A plugin for {plugin_name.lower()}"
        )
    
    def execute(self, assistant, *args, **kwargs):
        """Execute the {plugin_name.lower()} functionality"""
        print("🎯 {plugin_name} Plugin Executed!")
        
        # Add your plugin logic here
        # You can access assistant methods like:
        # assistant.analyze_code()
        # assistant.generate_code()
        
        # Example: Create a simple file
        content = f"# {plugin_name} Output\\n\\nThis file was created by the {plugin_name} plugin."
        filename = f"{file_name}_output.md"
        assistant.write_to_file(filename, content)
        
        return f"✅ {plugin_name} completed successfully. Output saved to {filename}"
...
'''
        ...
        plugin_path = os.path.join(self.plugin_manager.plugin_directory, f"{file_name}_plugin.py")
        try:
            with open(plugin_path, 'w', encoding='utf-8') as f:
                f.write(template_content.strip())
            print(f"✅ Plugin template created at {plugin_path}")
    ...
    def enhanced_menu(self):
        ...
        print("\n" + "="*80)
        print("🤖 Enhanced AI Coding Assistant v4.0 (Plugin System)")
        print("="*80)
        ...
        print("14. 🔌 Plugin System     - Manage and execute plugins")
        print("15. 💾 Export Session    - Save your work session")
        print("16. 🚪 Exit")
        print("="*80)
    
    def run(self):
        """Main assistant loop"""
        print("🚀 Starting Enhanced AI Coding Assistant v4.0...")
        print(f"📚 Config loaded: {len(self.config)} settings")
        print(f"🔌 Plugins loaded: {len(self.plugin_manager.plugins)} plugins")
